...
id_6980 <- function(conn, year) {
  ...
  df$Gesamtsteuerertrag <-
    df$Einkommen_Steuerbetrag_ktgde + df$Vermögen_Steuerbetrag_ktgde

  # Aggregate results per neighborhood
  df_result <- df %>%
    group_by(
      Wohnviertel_ID = wohnviertel_id_kdm,
  ...
  datei_pfad <- file.path(ordner_pfad, "6980.tsv")
  write.table(
    df_result,
    file = datei_pfad, sep = "\t",
    row.names = FALSE,
