...
parse_years <- function(x) {
  y <- as.numeric(strsplit(gsub(" ", "", x), ",")[[1]])
  y <- y[!is.na(y)]
  if (length(y) < 2) stop("❌ Mindestens zwei Jahre erforderlich.")
  sort(unique(y))
}

parse_tolerance <- function(x) {
  tol <- as.numeric(x)
  if (is.na(tol) || tol < 0) stop("❌ Toleranz muss numerisch und >= 0 sein (z.B. 10).")
  tol
}
...
add_diff_sheet <- function(wb, sheet, blocks, tolerance_pct) {
  ...
  for (nm in names(blocks)) {
    ...
    # Markierung: Werte > tol oder < -tol (ab Spalte 2)
    ...
    if (nrow(dat) > 0 && data_end_col >= data_start_col) {
      ...
      if (nrow(idx) > 0) {
        ...
        openxlsx::addStyle(
          wb, sheet, red_style,
          rows = rows_excel, cols = cols_excel,
...
if (length(args) < 2) stop("❌ Jahre und Toleranz fehlen (z.B. 2020,2021,2022 10)")

years <- parse_years(args[1])
tolerance_pct <- parse_tolerance(args[2])
