...
id_6909 <- function(conn, year){
  ...
  columns <- c("Einkommen_Steuerbetrag_ktgde", 
  ...
  calculate_revenue_bs <- function(df, year) {
    df %>%
      filter(Steuerjahr == year) %>%
      summarise(!!paste0("Ertrag Kt. BS ", year, " (rechte Skala)") :=
  ...
  df_final <- full_join(df_start, df_end, by = c("wohnviertel_id_kdm",
    ...
    select(
      ...
      paste0("Ertrag Kt. BS ", year_start, " (rechte Skala)"), 
      paste0("Ertrag Kt. BS ", year_end, " (rechte Skala)")
    )
